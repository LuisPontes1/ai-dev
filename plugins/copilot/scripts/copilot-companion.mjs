...
function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (copilotStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `gh auth login` to authenticate via GitHub CLI.");
  }
...
main().catch((error) => {
  ...
  if (isAuthError) {
    process.stderr.write(`Copilot authentication failed. Run \`gh auth login\` to authenticate and retry.\n`);
  } else {
